...
export const cruiseShipsService = {
  async getAll(page: number = 1, search?: string, cruiseLineId?: string): Promise<ApiResponse<CruiseShip>> {
    const params: any = { page }
    if (search) params.search = search
    if (cruiseLineId) params.cruise_line = cruiseLineId
    const response = await api.get<ApiResponse<CruiseShip>>('/api/cruises/cruise-ships/', { params })
    return response.data
  },
  async getById(id: string): Promise<CruiseShip> {
    const response = await api.get<CruiseShip>(`/api/cruises/cruise-ships/${id}/`)
    return response.data
  },
  async create(data: Partial<CruiseShip>): Promise<CruiseShip> {
    const response = await api.post<CruiseShip>('/api/cruises/cruise-ships/', data)
    return response.data
  },
  async update(id: string, data: Partial<CruiseShip>): Promise<CruiseShip> {
    const response = await api.patch<CruiseShip>(`/api/cruises/cruise-ships/${id}/`, data)
    return response.data
  },
  async delete(id: string): Promise<void> {
    await api.delete(`/api/cruises/cruise-ships/${id}/`)
  },
}
...
export const cruisePortsService = {
  async getAll(page: number = 1, search?: string): Promise<ApiResponse<CruisePort>> {
    const params: any = { page }
    if (search) params.search = search
    const response = await api.get<ApiResponse<CruisePort>>('/api/cruises/cruise-ports/', { params })
    return response.data
  },
  async getById(id: string): Promise<CruisePort> {
    const response = await api.get<CruisePort>(`/api/cruises/cruise-ports/${id}/`)
    return response.data
  },
  async create(data: Partial<CruisePort>): Promise<CruisePort> {
    const response = await api.post<CruisePort>('/api/cruises/cruise-ports/', data)
    return response.data
  },
  async update(id: string, data: Partial<CruisePort>): Promise<CruisePort> {
    const response = await api.patch<CruisePort>(`/api/cruises/cruise-ports/${id}/`, data)
    return response.data
  },
  async delete(id: string): Promise<void> {
    await api.delete(`/api/cruises/cruise-ports/${id}/`)
  },
}
