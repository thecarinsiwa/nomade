...
import { cruiseShipImagesService } from "@/lib/services/cruises"
import { CruiseShipImage } from "@/types"
import { useToast } from "@/hooks/use-toast"

interface CruiseShipImageGalleryProps {
  cruiseShipId: string
  readonly?: boolean
}
...
export function CruiseShipImageGallery({ cruiseShipId, readonly = false }: CruiseShipImageGalleryProps) {
  const [images, setImages] = useState<CruiseShipImage[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isAdding, setIsAdding] = useState(false)
  const [newImageUrl, setNewImageUrl] = useState("")
  const [newImageType, setNewImageType] = useState<CruiseShipImage['image_type']>('main')
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true)
      const data = await cruiseShipImagesService.getAll(cruiseShipId)
  ...
  const handleAddImage = async () => {
    ...
    try {
      setIsAdding(true)
      const newImage = await cruiseShipImagesService.create({
        cruise_ship: cruiseShipId,
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      await cruiseShipImagesService.delete(id)
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const updates = images
        ...
        .map(img => cruiseShipImagesService.update(img.id, { is_primary: false }))
      
      // Activer la nouvelle image principale
      await Promise.all([
        ...updates,
        cruiseShipImagesService.update(id, { is_primary: true })
      ])
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Images du navire</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Images du navire</span>
        </CardTitle>
        <CardDescription>
          Gérez les images associées à ce navire ({images.length} image{images.length > 1 ? 's' : ''})
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newImageType}
                  onValueChange={(value) => setNewImageType(value as CruiseShipImage['image_type'])}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue />
        ...
        {images.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>Aucune image pour ce navire</p>
            {!readonly && (
