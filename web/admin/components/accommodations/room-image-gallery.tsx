...
import { roomImagesService } from "@/lib/services/accommodations"
...
export function RoomImageGallery({ roomId, readonly = false }: RoomImageGalleryProps) {
  const [images, setImages] = useState<RoomImage[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isAdding, setIsAdding] = useState(false)
  const [newImageUrl, setNewImageUrl] = useState("")
  const [newImageType, setNewImageType] = useState<RoomImage['image_type']>('gallery')
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true)
      const data = await roomImagesService.getAll(roomId)
  ...
  const handleAddImage = async () => {
    ...
    try {
      setIsAdding(true)
      const newImage = await roomImagesService.create({
        room: roomId,
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      await roomImagesService.delete(id)
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const updates = images
        ...
        .map(img => roomImagesService.update(img.id, { is_primary: false }))
      
      // Activer la nouvelle image principale
      await Promise.all([
        ...updates,
        roomImagesService.update(id, { is_primary: true })
      ])
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Images de la chambre</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Images de la chambre</span>
        </CardTitle>
        <CardDescription>
          Gérez les images associées à cette chambre ({images.length} image{images.length > 1 ? 's' : ''})
        </CardDescription>
      </CardHeader>
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newImageType}
                  onValueChange={(value) => setNewImageType(value as RoomImage['image_type'])}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue />
