      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        ...
        <CollapsibleContent className="space-y-1">
          {item.children.map((child) => (
            <SidebarItem key={child.title} item={child} pathname={pathname} level={level + 1} />
          ))}
        </CollapsibleContent>
      </Collapsible>
    )
...
export function Sidebar({ className }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden lg:flex flex-col w-64 border-r bg-background h-[calc(100vh-4rem)] sticky top-16 overflow-y-auto",
        className
      )}
    >
      <div className="p-4 space-y-1">
        ...
        <div className="pt-4 space-y-1">
          {menuSections.map((section) => (
            <SidebarItem key={section.title} item={section} pathname={pathname} />
          ))}
        </div>
      </div>
    </aside>
