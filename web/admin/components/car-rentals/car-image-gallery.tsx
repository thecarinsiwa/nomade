...
import { carImagesService } from "@/lib/services/car-rentals"
...
const ANGLE_OPTIONS = [
  ...
  { value: 'front-side', label: 'Avant-côté' },
  { value: 'back-side', label: 'Arrière-côté' },
  { value: 'top', label: 'Dessus' },
  { value: 'interior', label: 'Intérieur' },
] as const

export function CarImageGallery({ carId, readonly = false }: CarImageGalleryProps) {
  const [images, setImages] = useState<CarImage[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isAdding, setIsAdding] = useState(false)
  const [newImageUrl, setNewImageUrl] = useState("")
  const [newImageType, setNewImageType] = useState<CarImage['image_type']>('main')
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true)
      const data = await carImagesService.getAll(carId)
  ...
  const handleAddImage = async () => {
    ...
    try {
      setIsAdding(true)
      const newImage = await carImagesService.create({
        car: carId,
        image_url: newImageUrl.trim(),
        image_type: newImageType,
        alt_text: newImageAlt.trim() || undefined,
        angle: newImageAngle.trim() || undefined,
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      await carImagesService.delete(id)
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const updates = images
        ...
        .map(img => carImagesService.update(img.id, { is_primary: false }))
      
      // Activer la nouvelle image principale
      await Promise.all([
        ...updates,
        carImagesService.update(id, { is_primary: true })
      ])
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Images du véhicule</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Gérez les images associées à ce véhicule ({images.length} image{images.length > 1 ? 's' : ''})
        </CardDescription>
      </CardHeader>
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newImageType}
                  onValueChange={(value) => setNewImageType(value as CarImage['image_type'])}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="image_angle">Angle de la photo (optionnel)</Label>
                <Select
                  value={newImageAngle || undefined}
                  onValueChange={(value) => setNewImageAngle(value === "none" ? "" : value)}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Sélectionner un angle" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="absolute top-2 right-2 flex space-x-2">
                ...
                {primaryImage.angle && (
                  <Badge variant="outline">
                    {ANGLE_OPTIONS.find(a => a.value === primaryImage.angle)?.label || primaryImage.angle}
                  </Badge>
                )}
              </div>
              {!readonly && (
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1">
                    ...
                    {image.angle && (
                      <Badge variant="outline" className="text-xs">
                        {ANGLE_OPTIONS.find(a => a.value === image.angle)?.label || image.angle}
                      </Badge>
                    )}
                  </div>
                  {!readonly && (
