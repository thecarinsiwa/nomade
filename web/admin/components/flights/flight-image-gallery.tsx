...
import { flightImagesService } from "@/lib/services/flights"
import { FlightImage } from "@/types"
...
const IMAGE_TYPES = [
  ...
  { value: 'cabin_economy', label: 'Cabine Économique' },
  { value: 'cabin_business', label: 'Cabine Business' },
  { value: 'cabin_first', label: 'Cabine Première Classe' },
  { value: 'meal', label: 'Repas' },
  { value: 'service', label: 'Service' },
  { value: 'other', label: 'Autre' },
] as const

export function FlightImageGallery({ flightId, readonly = false }: FlightImageGalleryProps) {
  const [images, setImages] = useState<FlightImage[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isAdding, setIsAdding] = useState(false)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [newImageUrl, setNewImageUrl] = useState("")
  const [newImageType, setNewImageType] = useState<FlightImage['image_type']>('aircraft')
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true)
      const data = await flightImagesService.getAll(flightId)
  ...
  const handleAddImage = async () => {
    ...
    try {
      setIsAdding(true)
      const newImage = await flightImagesService.create({
        flight: flightId,
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      await flightImagesService.delete(id)
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const updates = images
        ...
        .map(img => flightImagesService.update(img.id, { is_primary: false }))
      
      // Activer la nouvelle image principale
      await Promise.all([
        ...updates,
        flightImagesService.update(id, { is_primary: true })
      ])
  ...
  const handleUpdateImage = async (id: string, updates: Partial<FlightImage>) => {
    try {
      const updated = await flightImagesService.update(id, updates)
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Images du vol</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Images du vol</span>
        </CardTitle>
        <CardDescription>
          Gérez les images associées à ce vol ({images.length} image{images.length > 1 ? 's' : ''})
        </CardDescription>
      </CardHeader>
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newImageType}
                  onValueChange={(value) => setNewImageType(value as FlightImage['image_type'])}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue />
