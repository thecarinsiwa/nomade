...
import { airlineImagesService } from "@/lib/services/flights"
import { AirlineImage } from "@/types"
import { useToast } from "@/hooks/use-toast"

interface AirlineImageGalleryProps {
  airlineId: string
  readonly?: boolean
}
...
const IMAGE_TYPES = [
  ...
  { value: 'aircraft', label: 'Avion' },
  { value: 'cabin', label: 'Cabine' },
  { value: 'service', label: 'Service' },
  { value: 'other', label: 'Autre' },
] as const

export function AirlineImageGallery({ airlineId, readonly = false }: AirlineImageGalleryProps) {
  const [images, setImages] = useState<AirlineImage[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isAdding, setIsAdding] = useState(false)
  const [newImageUrl, setNewImageUrl] = useState("")
  const [newImageType, setNewImageType] = useState<AirlineImage['image_type']>('logo')
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true)
      const data = await airlineImagesService.getAll(airlineId)
  ...
  const handleAddImage = async () => {
    ...
    try {
      setIsAdding(true)
      const newImage = await airlineImagesService.create({
        airline: airlineId,
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      await airlineImagesService.delete(id)
  ...
  const handleSetPrimary = async (id: string) => {
    try {
      ...
      const updates = images
        ...
        .map(img => airlineImagesService.update(img.id, { is_primary: false }))
      
      // Activer la nouvelle image principale
      await Promise.all([
        ...updates,
        airlineImagesService.update(id, { is_primary: true })
      ])
  ...
  const logoImages = images.filter(img => img.image_type === 'logo' && !img.is_primary)
  const galleryImages = images.filter(img => img.image_type !== 'logo' && !img.is_primary)

  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Images de la compagnie</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Images de la compagnie</span>
        </CardTitle>
        <CardDescription>
          Gérez les images associées à cette compagnie aérienne ({images.length} image{images.length > 1 ? 's' : ''})
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newImageType}
                  onValueChange={(value) => setNewImageType(value as AirlineImage['image_type'])}
                  disabled={isAdding}
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {logoImages.map((image) => (
                <motion.div
                  ...
                  className="relative group border rounded-lg overflow-hidden bg-white p-4"
                >
                  <img
                    src={image.image_url}
        ...
        {images.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>Aucune image pour cette compagnie</p>
            {!readonly && (
