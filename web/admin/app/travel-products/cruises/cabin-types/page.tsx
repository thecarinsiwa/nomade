...
export default function CruiseCabinTypesPage() {
  ...
  const fetchCabinTypes = async () => {
    try {
      setIsLoading(true)
      const data = await cruiseCabinTypesService.getAll(1, searchTerm || undefined)
      ...
      <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground">
              Gestion des types de cabines (intérieure, extérieure, suite)
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Recherchez et gérez les types de cabines
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
                ...
                <TableBody>
                  {cabinTypes.length === 0 ? (
                    ...
                    cabinTypes.map((cabinType) => (
                      <TableRow key={cabinType.id}>
                        <TableCell className="font-medium">{cabinType.name}</TableCell>
                        <TableCell className="max-w-xs truncate">{cabinType.description || "-"}</TableCell>
                        <TableCell>{formatDateTime(cabinType.created_at)}</TableCell>
