...
export default function DeleteCruisePortPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      await cruisePortsService.delete(cruisePort.id)
      toast({
        title: "Succès",
      ...
      toast({
        title: "Erreur",
        description: "Impossible de supprimer le port",
        variant: "destructive",
      })
    } finally {
      setIsDeleting(false)
    ...
    <AdminLayout>
      <div className="space-y-8">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-destructive">Supprimer le port</h1>
          <p className="text-muted-foreground">
            Confirmez la suppression du port
          </p>
        </motion.div>
        ...
        <Card className="border-destructive">
          <CardHeader>
            ...
            <CardDescription>
              Cette action est irréversible. Le port et toutes ses données associées seront définitivement supprimés.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2">
                <p className="font-medium">{cruisePort.name}</p>
                {cruisePort.city && (
                  <p className="text-sm text-muted-foreground">
                    Ville: {cruisePort.city}
                  </p>
                )}
                {cruisePort.country && (
                  <p className="text-sm text-muted-foreground">
                    Pays: {cruisePort.country}
                  </p>
                )}
              </div>
            </div>
