...
export default function DeleteCarCategoryPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      await carCategoriesService.delete(category.id)
      toast({
        title: "Succès",
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="bg-muted p-4 rounded-lg">
                ...
                {category.cars_count && category.cars_count > 0 && (
                  <p className="text-sm text-muted-foreground mt-1">
                    Cette catégorie contient {category.cars_count} véhicule{category.cars_count > 1 ? "s" : ""}
                  </p>
                )}
              </div>
            </div>
