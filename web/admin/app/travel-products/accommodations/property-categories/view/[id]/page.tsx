...
export default function ViewPropertyCategoryPage() {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex space-x-2">
              <Link href={`/travel-products/accommodations/property-categories/edit/${category.id}`}>
                <Button>
                  <Edit className="mr-2 h-4 w-4" />
                  Modifier
                </Button>
              </Link>
              <Link href={`/travel-products/accommodations/property-categories/delete/${category.id}`}>
          ...
          <p className="text-muted-foreground">
            Informations complètes de {category.name}
          </p>
        </motion.div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{formatDateTime(category.created_at)}</p>
              </div>
              {category.description && (
                <div className="md:col-span-2">
                  <p className="text-sm text-muted-foreground mb-2">Description</p>
                  <p className="text-sm bg-muted p-3 rounded-lg">{category.description}</p>
                </div>
              )}
            </div>
          </CardContent>
