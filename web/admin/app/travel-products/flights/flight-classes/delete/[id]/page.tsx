...
export default function DeleteFlightClassPage() {
  ...
  useEffect(() => {
    const fetchFlightClass = async () => {
      try {
        ...
        toast({
          title: "Erreur",
          description: "Impossible de charger la classe",
          variant: "destructive",
        })
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      await flightClassesService.delete(flightClass.id)
      toast({
        title: "Succès",
    ...
    <AdminLayout>
      <div className="space-y-8">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-destructive">Supprimer la classe</h1>
          <p className="text-muted-foreground">
            Confirmez la suppression de la classe
          </p>
        </motion.div>
        ...
        <Card className="border-destructive">
          <CardHeader>
            ...
            <CardDescription>
              Cette action est irréversible. La classe sera définitivement supprimée.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="bg-muted p-4 rounded-lg">
                <p className="font-medium">{flightClass.name}</p>
              </div>
            </div>
