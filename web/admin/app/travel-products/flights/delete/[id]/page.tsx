...
export default function DeleteFlightPage() {
  ...
  useEffect(() => {
    const fetchFlight = async () => {
      try {
        ...
        toast({
          title: "Erreur",
          description: "Impossible de charger le vol",
          variant: "destructive",
        })
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      await flightsService.delete(flight.id)
      toast({
        title: "Succès",
    ...
    <AdminLayout>
      <div className="space-y-8">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-destructive">Supprimer le vol</h1>
        ...
        <Card className="border-destructive">
          <CardHeader>
            ...
            <CardDescription>
              Cette action est irréversible. Le vol et toutes ses données associées seront définitivement supprimés.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2">
                <p className="font-medium font-mono">{flight.flight_number}</p>
                <p className="text-sm text-muted-foreground">
                  {flight.departure_airport_name} → {flight.arrival_airport_name} | Statut: {flight.status}
                </p>
              </div>
            </div>
